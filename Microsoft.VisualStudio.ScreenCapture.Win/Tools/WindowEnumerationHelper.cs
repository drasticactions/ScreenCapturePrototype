...
namespace Microsoft.VisualStudio.ScreenCapture.Win
{
    ...
    public class WindowEnumerationHelper
    {
        ...
        public static IReadOnlyList<ProcessRecordSurface> GetMainCaptureSurfacesViaProcesses()
        {
            var windows = new List<ProcessRecordSurface>();
            ...
            foreach (var process in processes)
            {
                if (IsWindowValidForCapture(process.MainWindowHandle))
                {
                    try
                    {
                        windows.Add(new ProcessRecordSurface(process));
                    }
                    catch (Exception)
                    {
